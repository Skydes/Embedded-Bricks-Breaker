...
static void pb_ISR(void *arg) {
    ...
    if( (pb_state ^ prev_pb_state) & ((1 << PB_RIGHT)
                                      | (1 << PB_LEFT)
                                      | (1 << PB_CENTER)) ) {
...
void* thread_buttons() {
    ...
    while(1) {
        ...
        switch(game_state) {
            case WAITING:
                if(pb_center)
                    ball.angle = BAR_MIN_ANGLE
                                 + sys_xget_clock_ticks() % (BAR_MAX_ANGLE
                                                             - BAR_MIN_ANGLE);
            case PAUSED:
                if(pb_center) {
                    game_state = RUNNING;
...
void* thread_column(void *arg) {
    ...
    while(1) {
        ...
        /* Perform a rough check on the final position, save computation time */
        ...
        if(!out) {
            ...
            for(colli.iter = 0;
                    (colli.iter < iter_max) && !colli.happened; colli.iter++) {
                ...
                dist_x = ball_new.x - (BRICK_OFFSET + BRICK_W/2
                                       + idx*(BRICK_W + BRICK_OFFSET));
                if( (abs(dist_x) > (BRICK_W/2 + BALL_R))
                   || (ball_new.y > (NB_ROWS*(BRICK_OFFSET+BRICK_H)+BALL_R)) )
                    continue;
                ...
                for(row = 0; row < NB_ROWS; row++) {
                    if(bricks[idx][row] == BROKEN)
                        continue;
                    colli.happened = true;
                    dist_y = ball_new.y - (BRICK_OFFSET + BRICK_H/2
                                           + row*(BRICK_H + BRICK_OFFSET));
                    /* Bounce on vertical edge */
                    if( (abs(dist_x) == (BRICK_W/2 + BALL_R))
                            && (abs(dist_y) < BRICK_H/2)) {
                        colli.normal = dist_x > 0 ? 0 : 180;
                        break;
                    }
                    /* Bounce on horizontal edge */
                    if( (abs(dist_y) == (BRICK_H/2 + BALL_R))
                            && (abs(dist_x) < BRICK_W/2) ) {
        ...
        if(next_colli.happened && (next_colli.idx == idx)) {
            ...
            u8 inc = (bricks[idx][row] == GOLDEN) ? SCORE_INC_GOLDEN
                                                    : SCORE_INC_NORMAL;
...
void* thread_ball() {
    ...
    while(1) {
        ...
        if(game_state == RUNNING) {
            ...
            for(next_colli.iter = 0;
                    next_colli.iter < iter_max; next_colli.iter++) {
            ...
            for(u8 i  = 0; i < NB_COLUMNS; i++) {
                ...
                if( !column_colli[i].happened
                    || (column_colli[i].iter >= next_colli.iter) )
                    continue;
            ...
            if(next_colli.happened) {
                ...
                ball_new.angle = check_angle(2*next_colli.normal
                                             - (int) ball.angle + 180);
                ball_new.x = ball.x +round(next_colli.iter*cos(rad(ball.angle)));
                ball_new.y = ball.y +round(next_colli.iter*sin(rad(ball.angle)));
                
                /* Bouncing on bar: change velocity/angle but enforce limits */
                if(next_colli.idx == -1) {
                    if(offset_angle > 0)
                        ball_new.angle = min(ball_new.angle + offset_angle,
                                             BAR_MAX_ANGLE);
                    if(offset_angle < 0)
                        ball_new.angle = max(ball_new.angle + offset_angle,
                                             BAR_MIN_ANGLE);
                    if(offset_vel > 0)
                        ball_new.vel = min(ball_new.vel + offset_vel,
                                           BALL_MAX_VEL);
                    if(offset_vel < 0)
                        ball_new.vel = max(ball_new.vel + offset_vel,
                                           BALL_MIN_VEL);
                }

                /* Compute cos and sin in advance, save computation time */
            ...
            if( (bar.jump != NONE) || (bar.speed != 0)) {
                if( ((ball_new.y+BALL_R) >= (BZ_H-BAR_OFFSET_Y-BAR_H))
                    ...
                    bar.pos = ball_new.x
                              + ((bar.jump == RIGHT || bar.speed > 0) ? -1 : 1)
                              * (BALL_R + BAR_W/2);
                }
                bar.jump = NONE;
            }
        ...
        if( (game_state == RUNNING) && next_colli.happened)
            wait_time = (1000*next_colli.iter)/ball.vel
                        - (int) actual_time + (int) last_sent;
        else
...
void* main_prog(void *arg) {
    ...
    ret = XMbox_CfgInitialize(&mbx_display, configPtr_mailbox,
