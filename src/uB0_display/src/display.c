 ...
 * Description: High level drawing fucntions for each graphic object.
 *
 ******************************************************************************/
...
void display_msg(XTft *Tft, Game_state state) {
    ...
    drawBox(Tft, BZ_OFFSET_X+BZ_W/2-MSG_BOX_WIDTH/2,
                 BZ_OFFSET_X+BZ_W/2+MSG_BOX_WIDTH/2,
                 BZ_OFFSET_Y/2-MSG_BOX_HEIGHT/2,
                 BZ_OFFSET_Y/2+MSG_BOX_HEIGHT/2,
                 color, UNFILLED);
    while(buf[size++ +1] != '\0');
    writeText(Tft, BZ_OFFSET_X+(BZ_W-size*CHAR_W)/2,
              (BZ_OFFSET_Y-CHAR_H)/2, buf, color);
}
